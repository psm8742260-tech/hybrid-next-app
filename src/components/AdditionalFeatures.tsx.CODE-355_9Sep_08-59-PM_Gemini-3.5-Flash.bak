...
export function WorkerKYC({ kycState, onUpdateKYC, controlState = 'temp_on' }: KYCProps) {
  ...
  const handleAadhaarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        onUpdateKYC({ aadhaarUploaded: true, aadhaarImage: result });
      };
  ...
  const handlePanChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        const result = event.target?.result as string;
        setPanFile(result);
        onUpdateKYC({ panUploaded: true, panImage: result });
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleCardChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        setCardFile(result);
        if (cardType === 'labour') {
          onUpdateKYC({ labourCardUploaded: true, labourCardImage: result });
        } else {
          onUpdateKYC({ eshramCardUploaded: true, eshramCardImage: result });
        }
      };
      reader.readAsDataURL(file);
    }
  ...
  const triggerVerifyAll = () => {
    onUpdateKYC({ 
      ...
      idCardType: idTier,
      idCardNumber: `CWB-2026-${Math.floor(100000 + Math.random() * 900000)}`,
      ...
      {showIdCard && (
        <div className="mt-2 pt-3 border-t border-gray-100 space-y-3 animate-fade-in">
          ...
          <div className={`w-full max-w-sm mx-auto bg-gradient-to-br ${idBgGradient} text-white rounded-2xl p-4 shadow-2xl border-2 ${borderHighlight} relative overflow-hidden`}>
            ...
            {/* Top Bar of Card */}
            <div className="flex justify-between items-start border-b border-white/20 pb-1.5 mb-2.5">
              <div>
                <h3 className="text-[11px] font-black tracking-wider text-[#FFC000]">CWB సివిల్ వర్కర్స్ సమాఖ్య</h3>
                <p className="text-[6.5px] font-mono tracking-widest text-white/75 uppercase">CIVIL WORKER RELATION BOOK</p>
              </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="flex flex-col items-center">
                <div className="w-14 h-14 rounded-lg bg-slate-900 border border-white/30 flex items-center justify-center overflow-hidden relative shadow-md">
                  ...
                  <div className="absolute bottom-0 inset-x-0 bg-black/60 text-[6px] py-0.5 text-center font-bold text-amber-400 tracking-wider">
                    VERIFIED
                  </div>
                </div>
                <span className="text-[6px] font-mono text-white/60 mt-1 uppercase leading-none">ID CODE</span>
            ...
            {/* Card Footer Bar */}
            <div className="flex justify-between items-end mt-2 pt-1.5 border-t border-white/10">
              <div className="flex items-center gap-1">
                <div className="h-3 bg-white/20 w-12 rounded-xs flex items-center justify-between px-0.5 font-mono text-[4px] text-white/60">
                  ||||| | ||
                </div>
                <span className="text-[6px] text-white/40 font-mono">CWB SECURE DIGITAL</span>
              </div>
              <div className="text-[7px] font-extrabold text-amber-400 uppercase tracking-widest flex items-center gap-1">
