...
export const ManualFeatureTimer: React.FC<ManualFeatureTimerProps> = ({ featureId, initialHours = 24, onSave, type }) => {
    ...
    return (
      <button 
        ...
        className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-extrabold tracking-tight border transition-colors ${
          type === 'premium' ? 'bg-blue-50 border-blue-100 text-[#082c75] hover:bg-blue-100' : 'bg-amber-50 border-amber-200 text-amber-700 hover:bg-amber-100'
        }`}
      >
  ...
  return (
    <div className="flex items-center gap-1.5 bg-white border border-gray-200 p-1 rounded-lg shadow-sm">
      ...
      >
        <span className={`w-2.5 h-2.5 rounded-full bg-white transition-transform ${isOn ? "translate-x-3.5" : "translate-x-0.5"}`} />
      </button>
    </div>
  );
};
