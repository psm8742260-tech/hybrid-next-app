...
const RegistrationsManager: React.FC = () => {
  ...
  const handleCreateRegistration = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!addForm.fullName || !addForm.phoneNo) {
      alert("దయచేసి పేరు మరియు మొబైల్ నెంబర్ తప్పనిసరిగా ఎంటర్ చేయండి!");
      return;
    }

    try {
      const idCode = `CWB-2026-${Math.floor(100000 + Math.random() * 900000)}`;
      ...
      alert("కొత్త రిజిస్ట్రేషన్ విజయవంతంగా జోడించబడింది!");
    } catch (err) {
      console.error("Create Error:", err);
      alert("యాడ్ చేయడంలో లోపం తలెత్తింది.");
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (
      ...
      try {
        ...
        alert("తొలగించడంలో సమస్య ఏర్పడింది.");
      }
    }
  };
  ...
  const handleSaveEdit = async (id: string) => {
    try {
      ...
      alert("సవరించడంలో సమస్య ఏర్పడింది.");
    }
  };
    ...
    <div className="animate-fade-in space-y-4 max-h-[440px] overflow-y-auto pr-1 text-xs text-gray-700 font-sans">
      ...
      {showAddForm && (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              >
                ...
                <option value="customer">👤 కస్టమర్ (Customer Client)</option>
                <option value="vendor">🏪 వెండర్ (Hardware Shop)</option>
              </select>
            </div>
            <div>
              <label className="text-[9px] font-bold text-gray-500 block mb-1">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div>
              ...
              >
                ...
                <option value="SILVER">Silver (మధ్యస్థ)</option>
                <option value="GOLD">Gold (అనుభవం)</option>
                <option value="DIAMOND">Diamond (అత్యున్నత)</option>
              </select>
            </div>
          </div>
          ...
          <div className="flex gap-3 items-center border-t pt-3">
            <div className="flex-1">
              ...
              <input
                ...
                className="text-[10px] w-full block text-slate-500 file:mr-2 file:py-1 file:px-2 file:rounded-xl file:border-0 file:text-[9px] file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
              />
            </div>
            {addPhoto && (
              <div className="w-12 h-12 rounded-lg border overflow-hidden shrink-0 relative bg-white">
                ...
                <button
                  ...
                  className="absolute -top-1 -right-1 w-4 h-4 bg-red-600 text-white rounded-full flex items-center justify-center text-[8px] font-bold"
                >
                  ✕
                </button>
              </div>
          ...
          <button
            type="submit"
            className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl text-xs shadow-md transition-all active:scale-98"
          >
            సేవ్ చేయండి / Save User Registration 💾
          </button>
        </form>
      )}
      ...
      {filteredRegs.length === 0 ? (
        <div className="bg-white p-6 text-center border rounded-xl text-gray-400">
          ఈ కేటగిరీలో రిజిస్ట్రేషన్లు ఏవీ లేవు.
        </div>
      ) : (
        <div className="space-y-3">
          {filteredRegs.map((reg) => {
            const role = getResolvedRole(reg);
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="w-14 h-14 rounded-lg bg-gray-50 border border-gray-200 flex-shrink-0 overflow-hidden flex items-center justify-center relative">
                    ...
                    {reg.idCardType && (
                      <div className="absolute bottom-0 inset-x-0 bg-black/60 text-[6px] text-center text-amber-300 font-extrabold py-0.5 uppercase">
                        {reg.idCardType}
                      </div>
                    )}
                  </div>
                        ...
                        <div className="grid grid-cols-2 gap-1">
                          ...
                          <div>
                            ...
                            >
                              ...
                              <option value="customer">👤 కస్టమర్</option>
                              <option value="vendor">🏪 వెండర్</option>
                            </select>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-1">
                ...
                {!isEditing && (
                  <div className="flex items-center justify-between border-t border-gray-100 pt-2 mt-1">
                    <div className="flex gap-2">
                      {reg.aadhaarImage && (
                        <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-700 text-[8px] font-extrabold rounded border border-emerald-200">
                          Photo Verified ✓
                        </span>
                      )}
                      {reg.cardImage && (
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-2 py-1 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-700 rounded font-bold text-[9px] transition"
                      >
                        సవరించు / Edit
                      </button>
                      <button
                        onClick={() => handleDelete(reg.id)}
                        className="px-2 py-1 bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 rounded font-bold text-[9px] transition"
                      >
                        తొలగించు / Delete
                      </button>
                    </div>
