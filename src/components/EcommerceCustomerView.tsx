...
const mockProducts: Product[] = [
  {
    ...
    description:
      "Industrial grade safety helmet for construction workers. ISI certified.",
    price: 450,
    originalPrice: 600,
    discountPercentage: 25,
    images: [
      "https://images.unsplash.com/photo-1590432326759-90d40237e19b?w=400&q=80",
    ],
    sizes: ["Free Size"],
    colors: [
  ...
  },
  {
    ...
    description:
      "High visibility reflective jacket for night time work and road safety.",
    price: 250,
    originalPrice: 350,
    discountPercentage: 28,
    images: [
      "https://images.unsplash.com/photo-1621213032502-36fb88607dcb?w=400&q=80",
    ],
    sizes: ["M", "L", "XL", "XXL"],
    colors: [
  ...
  },
  {
    ...
    description:
      "Portable cement mixer 200L capacity. Ideal for small construction sites.",
    price: 15500,
    originalPrice: 18000,
...
interface EcommerceCustomerViewProps {
  onBack: () => void;
}

export default function EcommerceCustomerView({
  onBack,
}: EcommerceCustomerViewProps) {
  const [activeView, setActiveView] = useState<"list" | "detail" | "cart">(
    "list",
  );
  ...
  const addToCart = () => {
    ...
    setCart((prev) => {
      ...
      return [
        ...prev,
        { product: selectedProduct, quantity: 1, selectedSize, selectedColor },
      ];
    });
  ...
  const handleCheckout = () => {
    ...
    alert(
      `₹${total} కి ఆర్డర్ సిద్ధమవుతోంది... Instant Split Payment గేట్‌వే కి కనెక్ట్ అవుతోంది.`,
    );
    setCart([]);
    setActiveView("list");
  };
  ...
  const renderList = () => (
    <div className="space-y-4 animate-fade-in pb-20">
      <div className="bg-[#2563eb] -mx-4 -mt-4 p-4 pb-6 shadow-md rounded-b-3xl">
      ...
      <div className="grid grid-cols-2 gap-3">
        {filteredProducts.map((product) => (
          <div
            ...
            className="bg-white rounded-2xl p-2.5 shadow-sm border border-gray-100 flex flex-col active:scale-95 transition-transform"
          >
            ...
            <div className="mt-auto flex items-end justify-between">
              <div>
                ...
                {product.originalPrice && (
                  <span className="text-[10px] text-gray-400 line-through ml-1 block -mt-1">
                    ₹{product.originalPrice}
                  </span>
                )}
              </div>
              <button className="bg-[#FFC000] text-[#2563eb] p-1.5 rounded-lg shadow-sm">
    ...
    return (
      <div className="space-y-4 animate-fade-in pb-24">
        <button
          ...
          className="flex items-center gap-1 text-gray-500 font-bold text-xs bg-white py-1.5 px-3 rounded-full border shadow-sm w-max"
        >
        ...
        <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100">
          ...
          <div className="flex justify-between items-start mb-2">
            <h2 className="font-black text-lg text-gray-800 leading-tight flex-1 pr-2">
          ...
          <div className="flex items-baseline gap-2 mb-4">
            ...
            {selectedProduct.originalPrice && (
              <>
                ...
                <span className="text-xs font-black text-rose-500 bg-rose-50 px-1.5 py-0.5 rounded">
          ...
          {selectedProduct.sizes.length > 0 && (
            <div className="mb-5">
              <h4 className="font-extrabold text-xs text-gray-800 mb-2 uppercase tracking-wide">
                సైజు ఎంచుకోండి (Size)
              </h4>
              <div className="flex flex-wrap gap-2">
                {selectedProduct.sizes.map((size) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg font-bold text-xs border transition-colors ${selectedSize === size ? "bg-[#2563eb] text-white border-[#2563eb]" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"}`}
                  >
                    {size}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-3">
                {selectedProduct.colors.map((color) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-2 transition-transform ${selectedColor === color.name ? "scale-110 border-[#2563eb]" : "border-gray-200"}`}
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-40 sm:max-w-md sm:mx-auto">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 py-3.5 bg-[#FFC000] text-[#2563eb] font-black text-sm rounded-xl shadow-md active:scale-95 transition-transform"
            >
              కొనండి (Buy Now)
            </button>
          </div>
      ...
      {cart.length === 0 ? (
        ...
        <>
            ...
            {cart.map((item, idx) => (
              ...
              >
                ...
                <div className="flex-1">
                  <h4 className="font-bold text-xs text-gray-800 mb-1 leading-snug">
                    {item.product.name}
                  </h4>
          ...
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-2">
            <h4 className="font-black text-sm text-gray-800 mb-3 border-b pb-2">
              బిల్ వివరాలు (Bill Details)
            </h4>
            ...
            <div className="flex justify-between text-sm font-black text-[#2563eb] border-t pt-2 mt-2">
