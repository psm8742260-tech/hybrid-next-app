...
export default function EcommerceVendorDashboard() {
  const [activeTab, setActiveTab] = useState<"products" | "orders" | "finance">(
    "products",
  );
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    const newProduct = {
      ...
      image:
        "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&q=80", // default generic product image
    };
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-black text-gray-800 text-lg">
          మీ వస్తువులు (Products)
        </h3>
        <button
          onClick={() => setShowAddModal(true)}
          className="bg-[#2563eb] text-white px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1 shadow-md hover:bg-blue-900 transition-colors"
        >
      ...
      <div className="space-y-3">
        {products.length === 0 ? (
          ...
          products.map((product) => (
            <div
              key={product.id}
              className="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm flex gap-3 items-center"
            >
            ...
            <form onSubmit={handleAddProduct} className="p-4 space-y-3">
              <div>
                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wide mb-1">
                  వస్తువు పేరు / Product Name
                </label>
                <input
                  type="text"
                  value={newProdName}
                  onChange={(e) => setNewProdName(e.target.value)}
                  className="w-full p-2 bg-slate-50 border border-gray-200 rounded-xl text-xs font-bold focus:ring-1 focus:ring-[#2563eb] focus:outline-none"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 bg-slate-50 border border-gray-200 rounded-xl text-xs font-mono font-bold focus:ring-1 focus:ring-[#2563eb] focus:outline-none"
                    placeholder="0"
                  />
                </div>
                <div>
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-rose-50 border border-rose-100 p-4 rounded-3xl">
          <p className="text-[10px] font-bold text-rose-600 mb-1 uppercase">
            చెల్లించిన కమీషన్
          </p>
          <p className="text-xl font-black text-rose-800">
            ₹{vendorData.commissionPaid.toLocaleString()}
          </p>
        </div>
      </div>

      <div className="bg-[#2563eb] p-5 rounded-3xl text-white shadow-md relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 opacity-10">
          <TrendingUp className="w-24 h-24" />
        </div>
        <h4 className="font-bold text-xs text-white/70 uppercase tracking-wider mb-1">
          తదుపరి సెటిల్మెంట్
        </h4>
        <div className="text-3xl font-black mb-1">
          ₹{vendorData.pendingSettlement}
        </div>
        <p className="text-[10px] text-[#FFC000]">
          Instant Split Payment ద్వారా నేరుగా మీ ఖాతాకు వస్తుంది.
        </p>
      </div>

      <div className="mt-6">
        <h3 className="font-black text-gray-800 text-sm mb-3">
          ఇటీవలి ఆర్డర్లు (Recent Orders)
        </h3>
        <div className="space-y-3">
          {vendorData.recentOrders.map((order) => (
            <div
              key={order.id}
              className="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center"
            >
              <div>
  ...
  return (
    <div className="p-4 w-full">
      <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 mb-5 flex items-center gap-3">
        ...
        <div>
          <h2 className="font-black text-gray-800 leading-tight">
            {vendorData.name}
          </h2>
