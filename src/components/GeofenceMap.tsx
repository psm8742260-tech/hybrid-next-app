...
import { MapPin, Navigation, Settings, Eye, ShieldAlert, Sparkles, RefreshCw, Search } from 'lucide-react';
...
export default function GeofenceMap({ radiusKm = 10, onRadiusChange, controlState = 'temp_on' }: GeofenceMapProps) {
  const [localRadius, setLocalRadius] = useState(radiusKm);
  const [mapCenter, setMapCenter] = useState<[number, number]>([17.3850, 78.4867]); // Default Hyderabad, Telangana
  ...
  const [leads, setLeads] = useState([
    { id: 1, nameTe: "రాజు - ఎలక్ట్రీషియన్", nameEn: "Raju (Electrician)", dLat: 0.015, dLng: -0.02, phone: "9876543210" },
    { id: 2, nameTe: "సీత - పెయింటర్", nameEn: "Sita (Painter)", dLat: -0.045, dLng: 0.05, phone: "8765432109" },
    { id: 3, nameTe: "నాయుడు - మేస్త్రీ", nameEn: "Naidu (Mason)", dLat: 0.03, dLng: 0.025, phone: "7654321098" },
    { id: 4, nameTe: "రామ్ - ప్లంబర్", nameEn: "Ram (Plumber)", dLat: -0.01, dLng: -0.035, phone: "6543210987" },
    { id: 5, nameTe: "గోపి - కార్పెంటర్", nameEn: "Gopi (Carpenter)", dLat: 0.065, dLng: -0.01, phone: "9123456789" },
  ]);
  ...
  useEffect(() => {
    if (!mapLoaded || !mapInstanceRef.current || typeof L === 'undefined') return;
    ...
    const meIcon = L.divIcon({
      html: `<div class="relative flex items-center justify-center">
               ...
               <div class="relative rounded-full h-5.5 w-5.5 bg-[#082c75] border-2 border-white flex items-center justify-center shadow-lg">
    ...
    if (meMarkerInstanceRef.current) {
      ...
      meMarkerInstanceRef.current = L.marker(mapCenter, { icon: meIcon })
        .addTo(map)
        .bindPopup(`<div class="font-sans text-xs">
                     <p class="font-extrabold text-[#082c75]">నా ప్రస్తుత స్థానం / My Location</p>
    ...
    leads.forEach((lead) => {
      ...
      const distance = getDistance(mapCenter[0], mapCenter[1], leadLat, leadLng);
      ...
      const workerIcon = L.divIcon({
        html: `<div class="relative flex items-center justify-center">
                 ...
                 <div class="relative rounded-full h-7 w-7 ${
                   isInside ? 'bg-amber-500 border-2 border-white shadow-md text-amber-950 scale-110' : 'bg-slate-400 border border-white text-white opacity-70'
      ...
      const marker = L.marker([leadLat, leadLng], { icon: workerIcon })
        .addTo(map)
        .bindPopup(`<div class="font-sans text-xs space-y-1">
                     ...
                     <a href="tel:${lead.phone}" class="block mt-1 text-center py-1 bg-[#082c75] text-white text-[9px] font-bold rounded">
                       కాల్ చేయండి / Call
                     </a>
                   </div>`);
  ...
  if (controlState === 'temp_off' || controlState === 'perm_off') {
    return (
      <div className="bg-gray-100 rounded-2xl h-80 flex flex-col items-center justify-center border border-gray-200 text-center p-6">
        ...
        <h4 className="text-gray-900 font-bold text-base">జియోఫెన్సింగ్ మ్యాప్ నిలిపివేయబడింది</h4>
        <p className="text-red-600 text-xs font-semibold">Geofencing Map Suspended</p>
        <p className="text-gray-500 text-xs mt-2 max-w-sm">
          ఈ ఫీచర్ అడ్మినిస్ట్రేటర్ ద్వారా ప్రస్తుతం నిలిపివేయబడింది. దయచేసి అడ్మిన్ బోర్డు లో ఆన్ చేయండి.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-100 flex flex-col ${
      ...
      {/* Title Header bar */}
      <div className="bg-[#082c75] text-white px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Navigation className="w-5 h-5 text-[#FFC000] animate-bounce" />
          <div>
            <h4 className="font-bold text-sm">లైవ్ రియల్ మ్యాప్ (Real Live Map)</h4>
            <p className="text-[10px] text-gray-300">OpenStreetMap Interactive • {localRadius} KM Radius</p>
          </div>
        </div>
        {isUpgraded && (
          <span className="bg-[#FFC000] text-[#082c75] text-[9px] font-bold px-2 py-0.5 rounded flex items-center gap-1">
      ...
      <div className="p-3 bg-slate-50 border-b border-gray-100 flex flex-col gap-2">
        <form onSubmit={handleSearch} className="flex gap-1.5">
          <div className="relative flex-1">
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-xs bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-[#082c75]"
            />
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-[#082c75] hover:bg-[#001040] disabled:bg-gray-400 text-white font-extrabold text-xs rounded-lg transition-colors flex items-center gap-1 shrink-0"
          >
            {searchLoading ? <RefreshCw className="w-3 h-3 animate-spin" /> : 'వెతకండి (Search)'}
          </button>
        </form>

        {searchError && (
          <p className="text-[10px] text-rose-500 font-semibold px-1">{searchError}</p>
        )}
        ...
        <div className="flex flex-col gap-2 mt-2 w-full">
          <div className="flex items-center justify-between">
            <button
              ...
              className="px-2.5 py-1 text-[10px] font-black text-[#082c75] bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition flex items-center gap-1"
            >
              <MapPin className="w-3 h-3 text-blue-600" />
              <span>{geolocationLoading ? 'జిపిఎస్ వెతుకుతోంది...' : '📍 నా లైవ్ లొకేషన్ పట్టుకో (Locate Me)'}</span>
            </button>

            <span className="text-[9px] text-gray-500 font-mono flex items-center gap-1">
              <span className="font-bold text-[#082c75]">GPS:</span>
          ...
          <div className="bg-white p-2 rounded-lg border border-gray-200 flex items-center justify-between">
              <div className='flex items-center gap-2'>
                  <span className='font-bold text-[10px] text-gray-700'>ట్రాకర్ / Tracker:</span>
        ...
        {geolocationError && (
          <p className="text-[10px] text-amber-600 font-semibold px-1">{geolocationError}</p>
        )}
      </div>
      ...
      <div className="relative bg-[#f1f5f9] h-72 border-b border-gray-100 overflow-hidden">
        ...
        <div className="absolute left-3 bottom-3 bg-white/95 backdrop-blur-xs p-2 rounded-lg border border-gray-100 text-[9px] space-y-1 shadow-md" style={{ zIndex: 2 }}>
          <div className="flex items-center gap-1.5 text-gray-700">
            <span className="w-2.5 h-2.5 rounded-full bg-blue-500/25 border border-blue-500 block"></span>
            <span>పరిధి వలయం / Active Geofence boundary ({localRadius} KM)</span>
          </div>
          <div className="flex items-center gap-1.5 text-gray-700">
      ...
      <div className="p-3 bg-white border-b border-gray-100">
        <h5 className="font-extrabold text-xs text-gray-800 mb-2">సమీపంలోని వర్కర్లు (Nearby Service Workers)</h5>
        <div className="space-y-1.5 max-h-40 overflow-y-auto">
          {leads.map((lead) => {
            const leadLat = mapCenter[0] + lead.dLat;
            const leadLng = mapCenter[1] + lead.dLng;
            const dist = getDistance(mapCenter[0], mapCenter[1], leadLat, leadLng);
            ...
            return (
              <div
                ...
                className={`p-2 rounded-lg border text-xs flex items-center justify-between cursor-pointer transition hover:bg-slate-50 ${
                  insideFence ? 'border-emerald-200 bg-emerald-50/20' : 'border-gray-100 bg-white'
                }`}
              >
                <div>
                  <div className="font-bold text-gray-800 flex items-center gap-1">
                    <span>👷 {lead.nameTe}</span>
                    <span className="text-[10px] text-gray-500 font-normal">({lead.nameEn})</span>
                  </div>
                  <div className="text-[9px] text-gray-400">దూరం: {dist.toFixed(1)} కి.మీ.</div>
                </div>

                <div className="text-right">
                  <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${
                    insideFence ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-500'
                  }`}>
      ...
      <div className="p-4 bg-gray-50 flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="w-full flex-1">
          <div className="flex justify-between items-center mb-1">
            <span className="text-xs font-bold text-[#082c75]">కిలోమీటర్ల పరిధిని పెంచండి / Adjust Geofence Radius:</span>
            <span className="text-sm font-black text-[#082c75] font-mono bg-[#FFC000]/30 px-2 py-0.5 rounded">
              {localRadius} KM
            </span>
          </div>
          <input
            ...
            className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#082c75]"
          />
        </div>
      </div>
    </div>
