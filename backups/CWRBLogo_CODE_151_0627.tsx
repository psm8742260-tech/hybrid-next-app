...
export default function CWRBLogo({ 
  ...
  return (
    <div className={`flex ${stacked ? 'flex-col items-center text-center' : 'flex-col items-start justify-center'} ${className}`}>
      <div className={`overflow-hidden rounded-full flex items-center justify-center bg-[#072459] shadow-[0_0_25px_rgba(255,192,0,0.3)] border-4 border-[#FFC000] ${iconOnly ? 'w-full h-full' : 'w-14 h-14 sm:w-16 sm:h-16'}`}>
        {!imgError ? (
          <img 
            src={logo} 
            alt="CWRB Logo"
            className="w-full h-full object-cover scale-[1.22] object-center"
