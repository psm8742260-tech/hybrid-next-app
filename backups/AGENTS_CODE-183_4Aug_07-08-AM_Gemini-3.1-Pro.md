...
Whenever the agent proposes or makes a specific coding change, the agent MUST create a backup file and provide a unified Restore Code that strictly combines the Code ID, Date, precise current 12-Hour AM/PM Time (matching the exact system clock down to the minute with AM/PM), and the active Agent Name (e.g., `CODE-178_3Aug_09-23-AM_Gemini-3.6-Flash`). Do NOT generate restart/restore codes for casual conversations, questions, or when no code changes are made.
If the Admin later provides this exact Link Code, the agent MUST immediately retrieve the associated code snippet or state and restore/apply it as requested without needing further explanation. (Admin garu's strict instruction: "రీస్టార్ట్ కోర్టు టైమింగ్ ప్లస్ డేటు ప్లస్ ఏజెంట్ పేరు అన్ని కలిపి ఒకే కోడింగ్ లో రావాలి, కరెక్ట్ టైమింగ్ తో సహా ఉండాలి... మార్పులు చేర్పులు చేసినప్పుడు మాత్రమే").

**CRITICAL RULE: ASSETLINKS & FINGERPRINT PERMANENT LOCK (NEW)**
The Admin has placed a strict, permanent lock on the `public/.well-known/assetlinks.json` file. The package name (`studio.ai.hybridnext_v_0.twa`), the SHA-256 fingerprints, and the entire structure of this file MUST NEVER be changed, altered, or overwritten under any circumstances, even if the app version updates or changes. This is a permanent configuration. The agent must never modify this file or propose changes to it. (Admin garu's strict instruction: "మన యాప్ లో వర్షం మారిన ఫింగర్ ప్రింట్ గాని ఏది కూడా మారనే మారకూడదు పెర్మనెంట్ గా ఉండాలి ఆ విధంగా దాన్ని సెట్ చెయ్").
