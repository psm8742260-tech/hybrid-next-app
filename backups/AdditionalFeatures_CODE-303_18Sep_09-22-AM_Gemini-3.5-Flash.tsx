...
export function WalkieTalkie({ controlState = "temp_on" }: WalkieProps) {
    ...
    return (
      <div className="bg-slate-900 rounded-2xl p-6 text-center border-t-4 border-red-500 text-gray-400">
        ...
        <p className="text-xs text-red-400 font-semibold">
          Walkie-Talkie Suspended
        </p>
        <p className="text-gray-500 text-xs mt-2">
          అడ్మినిస్ట్రేటర్ ద్వారా ఈ ఫీచర్ ప్రస్తుతం అందుబాటులో లేదు.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`bg-slate-950 text-white rounded-2xl p-4 md:p-5 shadow-2xl relative overflow-hidden border ${
        isUpgraded
          ? "border-[#FFC000] shadow-[0_0_20px_rgba(255,192,0,0.15)]"
          : "border-slate-800"
      }`}
    >
      ...
      <div className="mb-3">
        ...
        <p className="text-[10px] text-gray-400">
          మొబైల్ ఫోన్ లాగా ఛానల్ నంబర్ డయల్ చేసి కనెక్ట్ అవ్వండి
        </p>
      </div>
      ...
      <div className="bg-emerald-950/40 border border-emerald-500/30 rounded-xl p-3 mb-4 font-mono relative overflow-hidden shadow-inner">
        ...
        <div className="flex justify-between items-start text-emerald-400">
          <div className="space-y-0.5">
            <span className="text-[8px] text-emerald-500/60 font-bold tracking-wider uppercase block">
              WALKIE STATUS
            </span>
            <div className="flex items-center gap-1">
              <span
                className={`w-1.5 h-1.5 rounded-full ${connectionStatus === "connected" ? "bg-emerald-400 animate-pulse" : connectionStatus === "dialing" ? "bg-amber-400 animate-ping" : "bg-gray-500"}`}
              />
          ...
          <div className="text-right">
            ...
            <span className="text-[10px] font-black tracking-wider text-emerald-300">
              {connectionStatus === "connected"
                ? `446.${connectedChannel.padStart(3, "0")} MHz`
                : "---.--- MHz"}
            </span>
          </div>
        </div>
        ...
        <div className="mt-2.5 flex justify-between items-end border-t border-emerald-500/10 pt-2">
          <div>
            <span className="text-[7.5px] text-emerald-500/50 block">
              CONNECTED TO:
            </span>
            <span className="text-[11px] font-extrabold text-emerald-200 tracking-wide block leading-none mt-1">
              {connectionStatus === "connected"
                ? getChannelInfo(connectedChannel)
                : "ఛానల్ ఎంచుకోండి / Enter Channel"}
            </span>
          </div>
          <div className="text-right pl-2">
            <span className="text-[8px] text-emerald-500/50 block font-mono">
              CHANNEL DIAL
            </span>
        ...
        <div className="col-span-7 bg-slate-900/60 p-2.5 rounded-xl border border-slate-800 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            ...
            <button
              ...
              className="h-9 rounded-lg bg-[#2563eb] hover:bg-[#002b80] text-[#FFC000] font-black text-[10px] transition active:scale-95 border border-blue-500/30 shadow-md cursor-pointer flex flex-col items-center justify-center leading-none"
            >
              ...
              {([5, 10, 20] as const).map((range) => (
                <button
                  ...
                  className={`w-full py-1 rounded text-[8.5px] font-bold transition-all ${
                    activeRange === range
                      ? "bg-[#FFC000] text-[#2563eb] font-black"
              ...
              className={`w-14 h-14 rounded-full flex flex-col items-center justify-center font-bold text-[8px] select-none shadow-lg transition-all active:scale-95 cursor-pointer ${
                connectionStatus !== "connected"
                  ...
                  : isTalking
                    ...
                    : "bg-[#FFC000] text-[#2563eb] border border-[#FFC000]/60 hover:bg-[#FFE060]"
              }`}
            >
              <Mic
                className={`w-4 h-4 mb-0.5 ${isTalking ? "animate-bounce" : ""}`}
              />
              <span className="leading-none tracking-tight text-center">
                PTT TALK
              </span>
            </button>
          </div>
      ...
      {isTalking && (
        <div className="mt-3 bg-emerald-500/10 border border-emerald-500/20 py-1.5 rounded-lg text-center animate-pulse">
          <span className="text-[10px] text-emerald-400 font-extrabold font-mono tracking-wider flex items-center justify-center gap-1">
            ...
            మాట్లాడండి... TRANSMITTING (CH {connectedChannel} • {activeRange}
            KM)...
          </span>
        </div>
      )}
      ...
      <p className="text-[9px] text-gray-500 text-center mt-2.5 leading-relaxed">
        నెంబర్ డయల్ చేసి కనెక్ట్ అయ్యాక,{" "}
        <strong className="text-gray-300">PTT TALK</strong> నొక్కి పట్టుకుని
        మాట్లాడవచ్చు.
      </p>
    </div>
  );
...
export function WorkerKYC({
  ...
  const [aadhaarFile, setAadhaarFile] = useState<string | null>(
    kycState.aadhaarImage || null,
  );
  const [panFile, setPanFile] = useState<string | null>(
    kycState.panImage || null,
  );
  ...
  const [regNumber, setRegNumber] = useState(
    kycState.registrationNumber || "LBR-5492-2018",
  );
  ...
  useEffect(() => {
    if (!kycState.verified) {
      const localDataStr =
        localStorage.getItem("cwb_worker_registration") ||
        localStorage.getItem("cwb_mobile_local_storage");
      if (localDataStr) {
        try {
          ...
          if (localData && localData.verified) {
            ...
            if (localData.registrationDate)
              setRegistrationDate(localData.registrationDate);
  ...
  let idBgGradient =
    "from-orange-50/50 via-orange-100/30 to-orange-100/10 text-slate-800";
  let borderHighlight = "border-orange-200/80";
  let badgeLabel = "🥉 BRONZE";

  if (expYears >= 10) {
    idTier = "diamond";
    idCardName = "డైమండ్ ఐడి కార్డ్ (Diamond Card)";
    idBgGradient = "from-sky-50/50 via-sky-100/30 to-sky-100/10 text-slate-800";
    borderHighlight = "border-sky-200";
    badgeLabel = "💎 DIAMOND";
  } else if (expYears >= 5) {
    idTier = "gold";
    idCardName = "బంగారు ఐడి కార్డ్ (Gold Card)";
    idBgGradient =
      "from-amber-50/50 via-yellow-100/30 to-yellow-100/10 text-slate-800";
    borderHighlight = "border-yellow-200/80";
    badgeLabel = "🥇 GOLD";
  } else if (expYears >= 3) {
    idTier = "silver";
    idCardName = "వెండి ఐడి కార్డ్ (Silver Card)";
    idBgGradient =
      "from-slate-50/50 via-slate-100/30 to-slate-100/10 text-slate-800";
  ...
  const handleCardChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        ...
        try {
          const res = await fetch("/api/extract-card-details", {
            method: "POST",
            headers: {
          ...
          if (data && !data.error) {
            ...
            if (data.phone) setPhoneNo(data.phone);

            if (cardType === "labour") {
              onUpdateKYC({
                labourCardUploaded: true,
                labourCardImage: compressed,
                issueDate: data.registrationDate,
                registrationNumber: data.registrationNumber,
              });
            } else {
              onUpdateKYC({
                eshramCardUploaded: true,
                eshramCardImage: compressed,
                issueDate: data.registrationDate,
                registrationNumber: data.registrationNumber,
              });
            }
          ...
          console.error("Error scanning card via OCR API:", err);
        } finally {
  ...
  const generateIdCardBackground = async () => {
    ...
    try {
      ...
      let gradStart = "#fcfdff";
      let gradMid = "#f4f9ff";
      let gradEnd = "#eaf3ff";
      let accentColor = "#0284c7"; // Solid sky blue
      let titleColor = "#2563eb"; // Dark blue for contrast
      ...
      const logoX = 75;
      const logoY = 55;
      const logoRad = 38; // Mockup perfect size
      let logoLoaded = false;
      try {
        const logoImg = await loadImage(
          "https://i.ibb.co/7JnVZGLw/1784961900190.png",
        );
        ctx.save();
        ctx.beginPath();
      ...
      // Header Texts (Stacked & Centered to match Mockup Card)
      ctx.textAlign = "center";
      ctx.fillStyle = titleColor;
      ctx.font = "900 17px sans-serif";
      ctx.fillText("CIVIL WORKER", 410, 36);
      ctx.fillText("CUSTOMER", 410, 56);
      ctx.fillText("RELATION BOOK", 410, 76);
      ...
      const frameX = 45;
      const frameY = 130;
      const frameW = 192; // 20% increase from 160
      const frameH = 240; // 20% increase from 200
      ...
      const userPhotoSrc =
        kycState.panImage || kycState.aadhaarImage || panFile || aadhaarFile;
      if (userPhotoSrc) {
        try {
      ...
      const shortId = kycState.idCardNumber
        ? kycState.idCardNumber.split("-")[2]
        : "827493";
      ...
      const detailsX = 265; // shifted to give room for 20% larger photo
      const detailsY = 140;
      ...
      drawDetailRow(
        "రిజిస్ట్రేషన్ నెంబర్ / Reg No",
        kycState.registrationNumber || regNumber,
        detailsX,
        detailsY + 100,
      );
      drawDetailRow(
        "సీనియారిటీ / Seniority",
        `${badgeText} (${expYears} Years)`,
        detailsX,
        detailsY + 150,
      );
      drawDetailRow(
        "రిజిస్ట్రేషన్ తేదీ / Reg Date",
        registrationDate,
        detailsX + 220,
        detailsY + 150,
      );
      ...
      const qrX = 45;
      const qrY = footerY + 12; // lowered slightly
      const qrSize = 74; // 28% increase from 58

      const verificationUrl =
        window.location.origin +
        "/?verify=true&id=" +
        (kycState.idCardNumber || `CWB-2026-${shortId}`);
      ...
      ctx.beginPath();
      ctx.roundRect
        ? ctx.roundRect(actX, actY, 110, 32, 8)
        : ctx.rect(actX, actY, 110, 32);
      ctx.fill();
    ...
    return (
      <div className="bg-white rounded-2xl p-6 text-center border border-gray-100 shadow-md">
        ...
        <h4 className="text-gray-900 font-bold">
          KYC వెరిఫికేషన్ నిలిపివేయబడింది
        </h4>
        <p className="text-xs text-red-500 font-semibold">
          KYC Verification Blocked
        </p>
        <p className="text-gray-500 text-xs mt-2">
          అడ్మిన్ ద్వారా ఈ విభాగం ప్రస్తుతం అందుబాటులో లేదు.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`bg-white rounded-2xl p-5 shadow-lg border border-gray-100 space-y-4 relative ${
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-gray-500">
          మీ లేబర్ కార్డు లేదా ఈ-శ్రమ కార్డు వివరాల ద్వారా అనుభవాన్ని
          నిర్ధారించి ఐడి పొందండి.
        </p>
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-xs font-bold text-gray-800">
              గూగుల్ వెరిఫికేషన్ / Google Auth
            </div>
            <div className="text-[10px] text-gray-500">
              {googleLoggedIn
                ? "ఆమోదించబడింది / Verified"
                : "వన్-క్లిక్ వెరిఫై"}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <div
            className={`h-24 rounded-xl border-2 border-dashed flex flex-col items-center justify-center overflow-hidden bg-gray-50 relative ${
              aadhaarFile
                ? "border-emerald-500"
                : "border-[#2563eb]/30 hover:border-[#2563eb]/60"
            }`}
          >
            {aadhaarFile ? (
              <>
                ...
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition">
                  <button
                    onClick={triggerMockAadhaar}
                    className="bg-white/90 text-[#2563eb] p-1.5 rounded-full shadow"
                  >
              ...
              <button
                type="button"
                onClick={triggerMockAadhaar}
                className="flex flex-col items-center text-[#2563eb]/70 p-2 text-center"
              >
        ...
        <div className="space-y-1.5">
          <span className="text-xs font-bold text-gray-600 block">
            పాస్పోర్ట్ ఫోటో / Passport Photo
          </span>
          <div
            className={`h-24 rounded-xl border-2 border-dashed flex flex-col items-center justify-center overflow-hidden bg-gray-50 relative ${
              panFile
      ...
      <div className="p-3.5 bg-amber-500/5 rounded-xl border border-amber-500/20 space-y-3">
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                className="w-full px-2 py-1 text-xs rounded border border-gray-300 bg-white text-gray-800 font-medium focus:ring-1 focus:ring-[#2563eb] outline-hidden"
              />
            </div>
            <div>
              <label className="text-[9px] font-bold text-gray-500 block mb-0.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[9px] font-bold text-gray-500 block mb-0.5">
                పని విభాగం (Profession)
              </label>
              <select
                value={profession}
                onChange={(e) => setProfession(e.target.value)}
                className="w-full px-1.5 py-1 text-xs rounded border border-gray-300 bg-white text-gray-800 font-medium focus:ring-1 focus:ring-[#2563eb] outline-hidden"
              >
                ...
                <option value="ఎలక్ట్రీషియన్ (Electrician)">
                  ఎలక్ట్రీషియన్ (Electrician)
                </option>
                <option value="కార్పెంటర్ (Carpenter)">
                  కార్పెంటర్ (Carpenter)
                </option>
                <option value="టైల్స్ మేస్త్రీ (Tiles Mason)">
                  టైల్స్ మేస్త్రీ (Tiles)
                </option>
              </select>
            </div>
            <div>
              <label className="text-[9px] font-bold text-gray-500 block mb-0.5">
                కార్డు రకం (Card Type)
              </label>
              <div className="flex bg-white rounded border border-gray-300 overflow-hidden text-[9px]">
                <button
                  ...
                  className={`flex-1 py-1 font-extrabold transition ${cardType === "labour" ? "bg-[#2563eb] text-white" : "text-gray-700 hover:bg-gray-50"}`}
                >
                  లేబర్
                </button>
                <button
                  ...
                  className={`flex-1 py-1 font-extrabold transition ${cardType === "eshram" ? "bg-[#2563eb] text-white" : "text-gray-700 hover:bg-gray-50"}`}
                >
                  ఈ-శ్రమ
                </button>
              </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <label className="text-[9px] font-bold text-gray-500 block mb-0.5">
                రిజిస్ట్రేషన్/జారీ తేదీ
              </label>
              <input
                type="date"
                value={registrationDate}
                onChange={(e) => setRegistrationDate(e.target.value)}
                className="w-full px-2 py-0.5 text-xs rounded border border-gray-300 bg-white text-gray-800 font-mono focus:ring-1 focus:ring-[#2563eb] outline-hidden"
              />
            </div>
            ...
            <div>
              ...
              >
                {isScanningCard
                  ...
                  : cardFile
                    ? "✓ కార్డ్ చేర్చబడింది"
                    : `+ అప్‌లోడ్ (${cardType === "labour" ? "లేబర్" : "ఈ-శ్రమ"})`}
              </button>
            </div>
          </div>
          ...
          <div className="p-3 bg-white border border-amber-500/20 rounded-xl shadow-xs space-y-1.5">
            <label className="text-[10px] font-black text-[#2563eb] flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#FFC000]"></span>
              <span>
                లేబర్ కార్డ్ రిజిస్ట్రేషన్ నెంబర్ / Registration Number
              </span>
            </label>
            <input
              type="text"
              ...
              className="w-full px-2.5 py-1.5 text-xs rounded border border-gray-300 bg-white text-gray-850 font-bold font-mono focus:ring-1 focus:ring-[#2563eb] outline-hidden placeholder:font-sans placeholder:font-normal"
            />
            <p className="text-[8.5px] text-gray-400 leading-tight">
              గమనిక: ఈ రిజిస్ట్రేషన్ నంబర్ ఆధారంగానే మీ డిజిటల్ ఐడెంటిటీ కార్డ్
              నంబర్ జారీ చేయబడుతుంది.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white/85 p-2 rounded-lg border border-amber-200 flex items-center justify-between text-[11px]">
          <div>
            <span className="font-bold text-gray-400 text-[8px] block uppercase">
              నిర్ధారించిన అనుభవం / Service:
            </span>
            <span className="font-black text-[#2563eb]">
              {expYears} సంవత్సరాలు ({expYears} Years)
            </span>
          </div>
          <div className="text-right">
            <span className="font-bold text-gray-400 text-[8px] block uppercase">
              జారీ అయ్యే ఐడి / ID Tier:
            </span>
            <span className="px-1.5 py-0.5 bg-[#2563eb] text-white font-extrabold text-[9px] rounded-sm uppercase tracking-wider block mt-0.5">
              {expYears >= 10
                ? "💎 DIAMOND"
                : expYears >= 5
                  ? "🥇 GOLD"
      ...
      <button
        ...
        className={`w-full py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-2 ${
          googleLoggedIn && aadhaarFile && panFile && cardFile
            ? "bg-[#2563eb] text-[#FFC000] hover:bg-[#2563eb]/95 shadow-md active:scale-[0.99] transition-all cursor-pointer"
      ...
      >
        ...
        <span>
          {kycState.verified
            ? "ఐడి కార్డు జారీ చేయబడింది / ID CARD ISSUED"
            : "ధృవీకరించి ఐడి కార్డ్ జారీ చేయి / Issue ID Card"}
        </span>
      </button>
      ...
      {showIdCard && (
        <div className="mt-2 pt-3 border-t border-gray-100 space-y-3 animate-fade-in">
          <div className="flex items-center justify-between">
            <h5 className="font-extrabold text-[11px] text-[#2563eb] uppercase tracking-wider flex items-center gap-1">
          ...
          <div
            className={`w-full max-w-sm mx-auto bg-gradient-to-br ${idBgGradient} rounded-2xl p-4 shadow-2xl border-2 ${borderHighlight} relative overflow-hidden text-slate-800`}
          >
            {/* Glossy Reflection overlay */}
            <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/5 to-white/10 pointer-events-none" />

            {/* Top Bar of Card */}
            <div className="flex items-center gap-2.5 border-b border-slate-300/60 pb-2 mb-2.5">
              {/* Logo (Bigger to match Mockup: w-14 h-14) */}
              <div className="w-14 h-14 rounded-full overflow-hidden bg-white border-2 border-amber-400 flex-shrink-0 flex items-center justify-center shadow-sm">
                <img
              ...
              {/* Stacked Centered Title (Mockup Perfect Layout) */}
              <div className="flex-1 text-center">
                <h3 className="text-[12px] sm:text-[12.5px] font-black tracking-wide text-[#2563eb] uppercase leading-[1.15] font-sans">
                  CIVIL WORKER <br />
                  CUSTOMER <br />
                  RELATION BOOK
                </h3>
              ...
              <div className="flex flex-col items-end flex-shrink-0">
                <span className="text-[6.5px] bg-slate-200/80 px-1.5 py-0.5 rounded-full font-black tracking-wider text-[#2563eb] border border-slate-300/40">
                  {badgeLabel}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              {/* Photo Area (20% bigger, vertical rectangle: w-[72px] h-[90px]) */}
              <div className="flex flex-col items-center flex-shrink-0">
                <div className="w-[72px] h-[90px] rounded-lg bg-slate-100 border border-slate-300/70 flex items-center justify-center overflow-hidden relative shadow-sm">
                  {panFile ? (
                    <img
                      src={panFile}
                      alt="Worker Profile"
                    ...
                    <img
                      src={kycState.aadhaarImage}
                      alt="Worker Profile"
                      className="w-full h-full object-cover"
                      referrerPolicy="no-referrer"
                    />
                  ) : (
                    <div className="w-12 h-12 rounded-full border border-amber-500 bg-white flex items-center justify-center text-xl shadow-xs">
                      👷
                    </div>
                  )}
                ...
                <span className="text-[5px] font-mono text-slate-500 mt-1 uppercase leading-none">
                  ID CODE
                </span>
                <span className="text-[8px] font-mono font-black text-slate-700">
                  #
                  {kycState.idCardNumber
                    ? kycState.idCardNumber.split("-")[2]
                    : "827493"}
                </span>
              </div>
              ...
              <div className="flex-1 space-y-1.5 text-[11px] text-slate-800">
                <div>
                  <span className="text-[7px] text-slate-500 block leading-none font-bold">
                    పేరు / Name
                  </span>
                  <span className="font-black text-[12.5px] text-slate-900 leading-tight block">
                    {fullName}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-1 pt-0.5">
                  <div>
                    ...
                    <span className="font-bold text-[9.5px] text-blue-800">
                      {profession.split(" ")[0]}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-bold text-[9.5px] text-slate-800">
                      {phoneNo}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <span className="font-bold font-mono text-[9.5px] text-red-700 block break-all leading-normal">
                    {kycState.registrationNumber || regNumber}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-1 pt-0.5">
                  <div>
                    ...
                    <span className="font-black text-[9.5px] text-emerald-700">
                      {badgeLabel} ({expYears} Years)
                    </span>
                  </div>
                  <div>
                    <span className="text-[7px] text-slate-500 block leading-none font-bold">
                    ...
                    <span className="font-bold font-mono text-[8px] text-slate-700">
                      {registrationDate}
                    </span>
                  </div>
                </div>
            ...
            <div className="flex justify-between items-center mt-2.5 pt-2 border-t border-white/10">
              <div className="flex items-center gap-2">
                {/* Real Scannable QR Code (20-30% bigger: w-12 h-12) */}
                <img
                  ...
                  alt="QR Code"
                  className="w-12 h-12 bg-white p-0.5 rounded shadow-sm"
                  referrerPolicy="no-referrer"
                />
                <div className="leading-tight">
                  <span className="text-[6px] text-slate-500 block font-mono font-bold">
                    CWRB OFFICIAL DIGITAL ID
                  </span>
                  <span className="text-[7.5px] font-mono font-black text-blue-800">
                    #
                    {kycState.idCardNumber
                      ? kycState.idCardNumber
                      : `CWB-2026-${kycState.idCardNumber ? kycState.idCardNumber.split("-")[2] : "827493"}`}
                  </span>
                </div>
              </div>
          ...
          {idCardDataUrl && (
            <div className="text-center max-w-sm mx-auto px-2">
              <p className="text-[9.5px] text-slate-600 font-bold bg-slate-100/90 py-1 px-3 rounded-lg border border-slate-200">
                💡 మొబైల్ లో సేవ్ అవ్వకపోతే కార్డు పై నొక్కి పట్టుకుని "Download
                Image" ఎంచుకోండి! (Long-press image to save)
              </p>
            </div>
          )}
          <div className="flex gap-2 justify-center max-w-sm mx-auto">
            ...
            <button
              onClick={() => {
                alert(
                  `ప్రింట్ ఆర్డర్ విజయవంతంగా స్వీకరించబడింది! మీ భౌతిక (Physical) ${idCardName} 7 రోజుల్లో మీ చిరునామాకు కొరియర్ ద్వారా పంపబడుతుంది.`,
                );
              }}
              className="px-3 bg-[#FFC000] hover:bg-[#FFE060] text-[#2563eb] py-1.5 rounded-lg text-[10px] font-extrabold transition active:scale-[0.98] cursor-pointer"
            >
...
export function WalletLedger({
  ...
  if (controlState === "temp_off" || controlState === "perm_off") {
    return (
      <div className="bg-white rounded-2xl p-6 text-center border border-gray-100 shadow-md">
        ...
        <p className="text-gray-500 text-xs mt-2">
          అడ్మినిస్ట్రేటర్ ద్వారా ఈ వాలెట్ ఫీచర్ ప్రస్తుతం ఆఫ్ చేయబడింది.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`bg-gradient-to-br from-[#2563eb] to-slate-900 text-white rounded-2xl p-5 shadow-xl relative overflow-hidden ${
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          <h4 className="font-bold text-xs text-gray-300">
            వర్కర్ డిజిటల్ వాలెట్ / Wallet Balance
          </h4>
        ...
        <button
          ...
          className="bg-[#FFC000] text-[#2563eb] font-bold text-[10px] px-3 py-1.5 rounded-lg hover:brightness-105 transition"
        >
          + 500 రీఛార్జ్ / Add
        </button>
      </div>
      ...
      <div className="border-t border-white/10 pt-3">
        ...
        <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
          {transactions.map((tx) => (
            <div
              key={tx.id}
              className="flex justify-between items-center text-xs bg-white/5 p-2 rounded-lg hover:bg-white/10 transition"
            >
              <div>
...
export function SubscriptionTiers({ controlState = "temp_on" }: SubProps) {
  const [selectedTier, setSelectedTier] = useState<
    "silver" | "gold" | "diamond" | null
  >(null);
  const [activeSubscription, setActiveSubscription] = useState<
    "silver" | "gold" | "diamond" | null
  >(() => {
    return (
      (localStorage.getItem("cwb_active_subscription") as
        "silver" | "gold" | "diamond" | null) || null
    );
  });
  ...
  const tiers = [
    {
      ...
      benefits: [
        "నెలవారీ 10 లీడ్స్",
        "పోస్ట్‌పెయిడ్ బీపీ & షుగర్ రిపోర్ట్",
        "ఆధార్ వెరిఫికేషన్",
      ],
    },
    {
      ...
      benefits: [
        "నెలవారీ 30 లీడ్స్",
        "స్మార్ట్ బయో-పవర్ + AI డాక్టర్",
        "24/7 పోస్ట్‌పెయిడ్ సపోర్ట్",
      ],
      popular: true,
    },
    {
      ...
      benefits: [
        "పరిమితి లేని లీడ్స్ & AI డాక్టర్",
        "రౌండ్ 25KM రేంజ్",
        "వాయిస్ వాకీటాకీ సపోర్ట్",
      ],
    },
  ];
  ...
  const handleApplyPromo = () => {
    ...
    if (code === "CWRB50") {
      ...
      setPromoMessage(
        "ప్రోమో కోడ్ విజయవంతంగా జోడించబడింది! 50% నెలవారీ ప్లాన్ తగ్గింపు వర్తించబడుతుంది.",
      );
    } else if (code === "FREE6") {
      setPromoApplied(true);
      setDiscountAmount(100);
      setPromoMessage(
        "ప్రోమో కోడ్ విజయవంతంగా జోడించబడింది! మొదటి 6 నెలలు పూర్తిగా ఉచితం!",
      );
    } else {
  ...
  return (
    <div
      className={`space-y-4 ${isUpgraded ? "p-3 bg-amber-50 rounded-2xl border border-amber-300" : ""}`}
    >
      <div className="flex justify-between items-start">
        <div>
          <h4 className="font-bold text-sm text-[#2563eb] flex items-center gap-1.5">
            <Landmark className="w-4.5 h-4.5 text-[#2563eb]" />
            <span>
              ప్రీమియం పోస్ట్‌పెయిడ్ సేవలు & బిల్లింగ్ కార్డులు / Postpaid
              Premium Plans & Billing
            </span>
          </h4>
          <p className="text-xs text-gray-500">
            మొదట వాడుకోండి - ఆపై చెల్లించండి! / Pay-After-Use Postpaid Billing
            Scheme!
          </p>
        </div>
        {activeSubscription && (
        ...
        <div className="p-4 bg-emerald-50 border border-emerald-300 rounded-2xl flex items-center justify-between shadow-xs">
            ...
            <div>
              ...
              <h5 className="font-extrabold text-xs text-emerald-950">
                {activeSubscription === "silver"
                  ? "సిల్వర్ పోస్ట్‌పెయిడ్ ప్లాన్ (Silver Postpaid Plan)"
                  : activeSubscription === "gold"
                    ? "గోల్డ్ పోస్ట్‌పెయిడ్ ప్లాన్ (Gold Postpaid Plan)"
                    : "డైమండ్ పోస్ట్‌పెయిడ్ ప్లాన్ (Diamond Postpaid Plan)"}
              </h5>
              <p className="text-[10px] text-emerald-800/90 leading-tight">
                బిల్లింగ్ స్థితి: మొదటి 3 నెలల ఉచిత ట్రయల్ (తదుపరి బిల్లింగ్
                తేదీ: 19 అక్టోబర్, 2026)
              </p>
            </div>
          </div>
          <div className="flex flex-col gap-1.5 items-end">
            <button
              ...
              className="px-2.5 py-1 bg-[#2563eb] text-white hover:bg-[#061e52] text-[9.5px] font-extrabold rounded-md transition"
            >
              రసీదు / Receipt
            </button>
            <button
            ...
            >
              {confirmCancel
                ? "నిజంగా రద్దు చేయాలా? (మళ్లీ నొక్కండి)"
                : "రద్దు చేయి"}
            </button>
          </div>
        </div>
        ...
        {tiers.map((tier) => {
          ...
          return (
            <div
              ...
              className={`cursor-pointer rounded-xl overflow-hidden border transition-all duration-300 relative ${
                isActive
                  ...
                  : isSelected
                    ? "border-[#2563eb] ring-4 ring-[#2563eb]/25 scale-[1.02]"
            ...
            >
              ...
              {/* Tier banner head */}
              <div
                className={`bg-gradient-to-r ${tier.color} text-white px-3 py-2 text-center relative`}
              >
                {tier.popular && (
                  <span className="absolute left-1.5 top-1.5 bg-[#2563eb] text-white text-[7px] font-black px-1 py-0.5 rounded">
                    POPULAR
                  </span>
                )}
                <div className="font-extrabold text-xs">{tier.nameTe}</div>
                <div className="text-[9.5px] opacity-90 font-bold">
                  {tier.nameEn}
                </div>
              </div>
                ...
                <ul className="mt-2.5 text-[10px] text-gray-600 text-left space-y-1.5 divide-y divide-gray-50">
                  {tier.benefits.map((b, idx) => (
                    <li key={idx} className="pt-1.5 flex items-center gap-1.5">
                      <Check
                        className={`w-3.5 h-3.5 shrink-0 ${isActive ? "text-emerald-600" : "text-[#2563eb]"}`}
                      />
      ...
      {selectedTier && !activeSubscription && (
        <motion.div
          ...
          className="p-4 bg-gradient-to-br from-[#2563eb]/5 to-[#2563eb]/10 border border-[#2563eb]/15 rounded-2xl space-y-3 shadow-xs"
        >
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[9px] font-black text-[#2563eb] bg-amber-400/25 px-2.5 py-0.5 rounded-full block w-fit mb-1">
                ఎంపిక చేసిన పోస్ట్‌పెయిడ్ ప్లాన్ / SELECTED POSTPAID PLAN
              </span>
              <h5 className="font-extrabold text-xs text-[#2563eb]">
                {selectedTier === "silver"
                  ? "సిల్వర్ పోస్ట్‌పెయిడ్ ప్లాన్ (Silver Postpaid)"
                  : selectedTier === "gold"
                    ? "గోల్డ్ పోస్ట్‌పెయిడ్ ప్లాన్ (Gold Postpaid)"
                    : "డైమండ్ పోస్ట్‌పెయిడ్ ప్లాన్ (Diamond Postpaid)"}
              </h5>
              <p className="text-[10px] text-gray-500 leading-tight">
                మొదటి 3 నెలలు పోస్ట్‌పెయిడ్ ట్రయల్ వ్యవధి పూర్తిగా ఉచితం (₹0)
              </p>
            </div>
            <div className="text-right">
              <span className="text-base font-black text-[#2563eb] block font-mono">
                {promoApplied && discountAmount === 100
                  ? "₹0"
                  : promoApplied
                    ? `₹${Math.floor(tiers.find((t) => t.id === selectedTier)!.priceVal * (1 - discountAmount / 100))}`
              ...
              <span className="text-[8.5px] text-emerald-600 font-extrabold block">
                వాడిన తర్వాతే చెల్లింపు
              </span>
            </div>
          </div>

          {/* Promocode section */}
          <div className="flex gap-2 items-center pt-2 border-t border-dashed border-[#2563eb]/10">
            <input
              type="text"
            ...
            <button
              ...
              className="px-3.5 py-1.5 bg-[#2563eb] hover:bg-[#061e52] text-white text-xs font-black rounded-lg transition"
            >
              అప్లై / Apply
            </button>
          </div>
          {promoMessage && (
            <p
              className={`text-[9px] font-bold ${promoApplied ? "text-emerald-600" : "text-rose-500"}`}
            >
              {promoMessage}
            </p>
          )}
          ...
          <button
            ...
            className="w-full py-3 bg-[#FFC000] hover:bg-[#e0a800] text-[#2563eb] font-black text-xs rounded-xl shadow-md transition flex items-center justify-center gap-2"
          >
            {isSubscribing ? (
              <>
              ...
              <span>
                పోస్ట్‌పెయిడ్ ఉచితంగా ప్రారంభించండి / Start Free Postpaid Now
              </span>
            )}
          </button>
        </motion.div>
      ...
      {showReceipt && activeSubscription && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          ...
          >
            {/* Receipt Header */}
            <div className="bg-[#2563eb] text-white p-5 text-center relative">
              <button
              ...
              <h4 className="font-extrabold text-sm tracking-tight">
                పోస్ట్‌పెయిడ్ సేవలు & బిల్లింగ్ రసీదు / Invoice
              </h4>
              <p className="text-[9px] text-white/70">
                CWRB MULTIPURPOSE SERVICES PVT LTD
              </p>
            </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="text-[10.5px] space-y-2 border-b pb-3 border-dashed">
                <div className="flex justify-between">
                  <span className="text-gray-400">
                    రసీదు సంఖ్య / Invoice No:
                  </span>
                  <span className="font-mono font-bold">
                    INV-POST-2026-{Math.floor(100000 + Math.random() * 900000)}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-400">
                    చెల్లింపు పద్ధతి / Payment:
                  </span>
                  <span className="font-bold text-emerald-600">
                    పోస్ట్‌పెయిడ్ ట్రయల్ (Trial ₹0)
                  </span>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-xs font-black text-[#2563eb]">
                    {activeSubscription === "silver"
                      ? "సిల్వర్ పోస్ట్‌పెయిడ్ ప్లాన్"
                      : activeSubscription === "gold"
                        ? "గోల్డ్ పోస్ట్‌పెయిడ్ ప్లాన్"
                        : "డైమండ్ పోస్ట్‌పెయిడ్ ప్లాన్"}
                  </span>
                ...
                <p className="text-[9px] text-gray-500">
                  3 నెలల ఉచిత ట్రయల్ పోస్ట్‌పెయిడ్ వ్యవధి ముగిసేవరకు మీకు
              ...
              <div className="space-y-1.5 text-xs pt-1.5">
                ...
                {promoApplied && (
                  <div className="flex justify-between text-emerald-600 font-bold text-[10.5px]">
                    <span>
                      ప్రోమో తగ్గింపు / Promo Added ({discountAmount}%)
                    </span>
                    <span className="font-mono">వర్తించబడింది</span>
                  </div>
                )}
                <div className="flex justify-between font-black text-[#2563eb] border-t pt-2 mt-2 text-sm">
              ...
              <p className="text-[8.5px] text-gray-400 leading-tight text-center">
                తదుపరి బిల్లింగ్ తేదీ 19 అక్టోబర్, 2026 నుండి ప్లాన్ రుసుము
                వర్తిస్తుంది. మీరు ఏ సమయంలోనైనా సెట్టింగ్స్ ద్వారా దీనిని రద్దు
                చేసుకోవచ్చు.
              </p>
              ...
              <button
                onClick={() => setShowReceipt(false)}
                className="w-full py-2 bg-[#2563eb] hover:bg-[#061e52] text-white font-black text-xs rounded-xl shadow-md transition"
              >
                పూర్తయింది / Done
              </button>
            </div>
    ...
    return (
      <div className="bg-white rounded-2xl p-6 text-center border text-gray-400">
        ...
        <h4 className="text-gray-800 font-bold">
          వ్యక్తిగత డైరీ నిలిపివేయబడింది
        </h4>
        <p className="text-xs text-red-500 font-semibold">
          Expense Diary Suspended
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div>
        ...
        <p className="text-xs text-gray-500">
          వర్కర్ల ఆదాయం మరియు రోజువారీ ఖర్చుల డైరీ
        </p>
      </div>
      ...
      >
        <div className="grid grid-cols-3 gap-2">
          <div className="col-span-2">
            <input
              ...
              className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-1 focus:ring-[#2563eb]"
            />
          </div>
          <div>
            <input
              ...
              className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none font-mono focus:ring-1 focus:ring-[#2563eb]"
            />
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            type="submit"
            className="px-4 py-1 bg-[#2563eb] text-white rounded-lg text-[10px] font-bold flex items-center gap-1 hover:bg-[#2563eb]/95"
          >
      ...
      <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
        {entries.map((entry) => (
          <div
            key={entry.id}
            className="flex justify-between items-center text-xs p-2 bg-white rounded-lg border border-gray-100 hover:bg-gray-50 transition"
          >
            <div className="flex items-center gap-2">
              {entry.type === "income" ? (
...
export function BioPowerManager({
  ...
  return (
    <div
      className={`bg-slate-900 border ${isResting ? "border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.15)]" : "border-slate-800"} rounded-2xl p-4 overflow-hidden relative`}
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div>
          <h3 className="font-black text-white text-sm flex items-center gap-1.5">
            <Zap
              className={`w-4 h-4 ${isResting ? "text-emerald-400" : "text-[#FFC000]"}`}
            />
            <span>బయో-ఎనర్జీ మానిటర్</span>
          </h3>
          <p className="text-[9px] text-slate-400 mt-0.5 uppercase tracking-wide">
            Smart Bio-Power Manager
          </p>
        </div>

        <div
          className={`px-2 py-1 rounded-full text-[9px] font-bold flex items-center gap-1 border ${isResting ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" : "bg-slate-800 text-slate-400 border-slate-700"}`}
        >
      ...
      {controlState === "temp_off" || controlState === "perm_off" ? (
        <div className="py-8 text-center bg-slate-950 rounded-xl border border-slate-800 p-4 flex flex-col items-center justify-center space-y-3">
          ...
          <div className="space-y-1">
            <h4 className="text-slate-300 font-bold text-xs">
              మానిటర్ ఆఫ్ చేయబడింది (Monitor OFF)
            </h4>
            <p className="text-[10px] text-slate-500">
              ఈ సేవ ప్రస్తుతం నిలిపివేయబడింది. దీనిని ఇప్పుడే ఆక్టివేట్
              చేసుకోండి.
            </p>
          </div>
          {onToggleActive && (
            ...
            >
              <Zap className="w-3.5 h-3.5 animate-pulse" /> మానిటర్ ఆన్ చేయి
              (Turn Monitor ON)
            </button>
          )}
        </div>
        ...
        <div className="space-y-4 relative z-10">
          ...
          <div className="bg-slate-950 p-3 rounded-xl border border-emerald-500/30 space-y-3 relative overflow-hidden">
            ...
            <div className="text-[9.5px] text-emerald-400 font-bold flex items-center gap-1">
              ...
              స్మార్ట్ బయో-ఎనర్జీ మానిటర్ (Smart Bio-Energy Monitor)
            </div>
            ...
            <div className="flex gap-3">
              <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 flex-1 relative overflow-hidden">
                <div className="text-[9px] text-slate-400 mb-1 flex items-center gap-1">
                  <Heart className="w-3 h-3 text-rose-500 animate-pulse" /> హృదయ
                  స్పందన (BPM)
                </div>
              ...
              <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 flex-1 relative overflow-hidden">
                <div className="text-[9px] text-slate-400 mb-1 flex items-center gap-1">
                  <Battery className="w-3 h-3 text-emerald-400" /> బ్యాటరీ ఆదా
                  (Saved)
                </div>
                <div className="text-2xl font-black font-mono text-emerald-400 flex items-end gap-1">
                  {energySaved.toFixed(1)}{" "}
                  <span className="text-[10px] text-emerald-600 font-bold mb-1">
                    mWh
                  </span>
                </div>
              </div>
                ...
                {history.map((val, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      style={{
                        opacity: i === history.length - 1 ? 1 : 0.5 + i * 0.03,
                      }}
                    />
                  );
                })}
            ...
            <button
              onClick={() =>
                alert(
                  "ఈ బయో ఎనర్జీ సేవలు పోస్ట్ పెయిడ్ వినియోగదారులకు (Postpaid Users) మాత్రమే అందుబాటులో ఉంటాయి! / Premium Postpaid Feature Only!",
                )
              }
            ...
            >
              ఈ సదుపాయాలన్నీ పోస్ట్ పెయిడ్ వినియోగదారులకు మాత్రమే (Postpaid
              Users Only)
            </button>
          </div>
          ...
          <div className="bg-slate-950 p-3 rounded-xl border border-sky-500/30 space-y-2 relative overflow-hidden">
            ...
            <div className="text-[9px] text-sky-400 font-bold flex items-center gap-1">
              <Activity className="w-3 h-3 text-sky-400 animate-pulse" /> హార్ట్
              రేట్ ఆధారిత రక్తపోటు & షుగర్ అంచనా
            </div>

            <div className="grid grid-cols-2 gap-2 text-center">
              <div className="bg-slate-900/60 p-2 rounded-lg border border-slate-800">
                <span className="text-[8.5px] text-slate-400 block mb-0.5">
                  రక్తపోటు (Blood Pressure)
                </span>
                <span className="text-[11px] sm:text-xs font-black font-mono text-white">
                  {110 + Math.floor((bpm - 60) * 0.5)}/
                  {70 + Math.floor((bpm - 60) * 0.3)}{" "}
            ...
            <button
              onClick={() =>
                alert(
                  "ఈ సదుపాయాలన్నీ పోస్ట్ పెయిడ్ వినియోగదారులకు (Postpaid Users) మాత్రమే అందుబాటులో ఉంటాయి! / Premium Postpaid Feature Only!",
                )
              }
          ...
          <div className="bg-slate-950 p-3 rounded-xl border border-rose-500/30 space-y-3 relative overflow-hidden">
            ...
            <div className="text-[9.5px] text-rose-400 font-bold flex items-center gap-1">
              <Camera className="w-3.5 h-3.5 text-rose-400 animate-pulse" /> AI
              డాక్టర్ ఫేస్ హెల్త్ స్కానర్ (Doctor Scan)
            </div>

            {scanStep === "idle" && (
              <div className="space-y-2">
                <p className="text-[9px] text-slate-400 leading-normal">
                  వ్యక్తి ఫోటోను తీయడం ద్వారా కృత్రిమ మేధస్సు (AI) తో వారి
                  వాతావరణ ఆధారిత ఆరోగ్య సమస్యలను, అలసట స్థాయిలను మరియు తక్షణ
                  ఉపశమన సలహాలను డాక్టర్ లాగా విశ్లేషిస్తుంది.
                </p>
                <button
                ...
                >
                  <Camera className="w-3 h-3" /> ఫేస్ హెల్త్ స్కాన్ ప్రారంభించు
                  (Start AI Scan)
                </button>
              </div>
            )}
            ...
            {scanStep === "capturing" && (
              <div className="bg-slate-900 rounded-lg p-3 text-center space-y-2 relative border border-rose-500/20 overflow-hidden">
                ...
                <div className="text-[10px] text-rose-300 font-bold animate-pulse">
                  కెమెరా ఆన్ అవుతోంది... ముఖాన్ని స్క్రీన్ లో ఉంచండి (
                  {scanProgress}%)
                </div>
            ...
            {scanStep === "analyzing" && (
              <div className="bg-slate-900 rounded-lg p-4 text-center space-y-2 border border-yellow-500/20">
                ...
                <div className="text-[10px] text-yellow-300 font-bold animate-pulse">
                  AI డాక్టర్ విశ్లేషిస్తున్నారు / Diagnosing symptoms with AI
                  Doctor...
                </div>
              </div>
            )}
              ...
              <div className="bg-slate-900 rounded-lg p-3 border border-emerald-500/20 space-y-2.5">
                <div className="flex items-center justify-between text-emerald-400 text-[9.5px] font-bold pb-1 border-b border-slate-800">
                  <span className="flex items-center gap-1">
                    ...
                    AI డాక్టర్ రిపోర్ట్ (Certified AI Prescription)
                  </span>
                  <span className="text-[8px] bg-emerald-500/10 px-1.5 py-0.5 rounded text-emerald-300">
                    పూర్తయింది
                  </span>
                </div>
                ...
                <div className="space-y-2 text-[9.5px]">
                  <div>
                    <span className="text-slate-400 block font-bold">
                      ఆరోగ్య పరిస్థితి / Diagnostic View:
                    </span>
                    <p className="text-white font-semibold">
                      ఎండ తీవ్రత వల్ల శరీర ఉష్ణోగ్రత పెరిగి, కళ్ళు ఎర్రబడటం
                      మరియు తేలికపాటి డీహైడ్రేషన్ (అలసట) సంకేతాలు కనపడుతున్నాయి.
                    </p>
                  </div>
                  ...
                  <div>
                    <span className="text-amber-400 font-bold block">
                      వైద్యుని ఉచిత సలహా & మందులు (Remedies & Prescription):
                    </span>
                    <ul className="list-disc list-inside text-slate-200 space-y-1 mt-1 font-medium">
                      <li>
                        వెంటనే O.R.S (ఓరల్ రీహైడ్రేషన్ సాల్ట్) లేదా గ్లూకోజ్
                        నీరు త్రాగాలి.
                      </li>
                      <li>
                        తలనొప్పి లేదా జ్వరం ఉంటే నిమ్మరసం లేదా పారాసిటమాల్ 500mg
                        వాడండి.
                      </li>
                      <li>
                        కనీసం 30 నిమిషాల పాటు చల్లని నీడ గల ప్రదేశంలో విశ్రాంతి
                        తీసుకోండి.
                      </li>
                    </ul>
                  </div>
                ...
                <button
                  onClick={() =>
                    alert(
                      "ఈ పూర్తి మెడికల్ సర్టిఫికేషన్ మరియు నివేదిక డౌన్‌లోడ్ కేవలం పోస్ట్ పెయిడ్ వినియోగదారులకు (Postpaid Users) మాత్రమే అందుబాటులో ఉంటుంది! / Premium Postpaid Feature Only!",
                    )
                  }
          ...
          <div className="bg-slate-950 p-3 rounded-xl border border-teal-500/30 space-y-3 relative overflow-hidden">
            ...
            <div className="text-[9.5px] text-teal-400 font-bold flex items-center gap-1">
              ...
              వాతావరణ రేడియో నివేదిక (RF Weather Report)
            </div>

            <p className="text-[9px] text-slate-400 leading-normal">
              ప్రస్తుత వాతావరణ పరిస్థితులు మరియు వేడి గాలి తీవ్రత ఆధారంగా
              కార్మికుల శరీర రక్షణకు అవసరమైన సూచనలు.
            </p>
            ...
            <button
              onClick={() =>
                alert(
                  "ఈ వాతావరణ నివేదిక సేవలు పోస్ట్ పెయిడ్ వినియోగదారులకు (Postpaid Users) మాత్రమే అందుబాటులో ఉంటాయి! / Premium Postpaid Feature Only!",
                )
              }
          ...
          <div className="text-[8px] text-slate-500 text-center leading-relaxed">
            *గమనిక: యూザー విశ్రాంతి తీసుకుంటున్నప్పుడు (BPM &lt; 80)
            బ్యాక్‌గ్రౌండ్ ఆక్టివిటీ తగ్గి, బ్యాటరీ ఆదా అవుతుంది.
          </div>
        </div>
      )}
    </div>
