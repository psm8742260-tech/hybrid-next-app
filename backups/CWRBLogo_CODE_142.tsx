...
export default function CWRBLogo({ 
  ...
  return (
    <div className={`flex ${stacked ? 'flex-col items-center text-center' : 'flex-col items-start justify-center'} ${className}`}>
      <div className={`flex items-center justify-center ${iconOnly ? 'w-full h-full' : 'w-14 h-14 sm:w-16 sm:h-16'}`}>
        {!imgError ? (
          <img 
            src={logo} 
            alt="CWRB Logo"
            className="w-full h-full object-contain scale-[1.0] object-center mix-blend-lighten"
